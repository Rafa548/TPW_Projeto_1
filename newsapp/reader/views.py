...
from .forms import NewsSaveForm
...
def home(request):
    ...
    if context is None:
        ...
        while r.status_code == 429:
            print("Request failed. Changing API key")
...
def category(request):
    ...
    if context is None:
        if request.method == "GET":
            ...
            while r_trending.status_code == 429:
                print("Request failed. Changing API key")
                API_KEY = select_new_api_key()
                trending_url = "https://newsapi.org/v2/top-headlines?sortBy={}&country=us&page={}&apiKey={}".format(
                    "popularity", 1, API_KEY
            ...
            while r_latest.status_code == 429:
                print("Request failed. Changing API key")
                API_KEY = select_new_api_key()
                latest_url = "https://newsapi.org/v2/top-headlines?country=us&page={}&apiKey={}&sortBy={}".format(
                    1, API_KEY, "publishedAt"
            ...
            while r_popular.status_code == 429:
                print("Request failed. Changing API key")
                API_KEY = select_new_api_key()
                popular_url = "https://newsapi.org/v2/everything?q={}&page={}&apiKey={}&sortBy={}".format(
                    query, 1, API_KEY, "popularity"
                ...
                context["Trending_Data"].append(
                    {
                        ...
                        "description": ""
                        if i_trending["description"] is None
                        else i_trending["description"],
                        "url": i_trending["url"],
                        "image": temp_img
                        if i_trending["urlToImage"] is None
                        else i_trending["urlToImage"],
                ...
                context["Latest_Data"].append(
                    {
                        ...
                        "description": ""
                        if i_latest["description"] is None
                        else i_latest["description"],
                        "url": i_latest["url"],
                        "image": temp_img
                        if i_latest["urlToImage"] is None
                        else i_latest["urlToImage"],
                ...
                context["Popular_Data"].append(
                    {
                        ...
                        "description": ""
                        if i_popular["description"] is None
                        else i_popular["description"],
                        "url": i_popular["url"],
                        "image": temp_img
                        if i_popular["urlToImage"] is None
                        else i_popular["urlToImage"],
